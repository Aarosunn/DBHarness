...
for r in 1 2 3; do
    uv run python run.py --engines postgres,sqlalchemy,neo4j --workloads feed \
        --requests 500 --warmup 25 --name leaf0729_baselines_r$r
    uv run python run.py --engines jaseci --url-jaseci http://localhost:8005 \
        --workloads feed --requests 500 --warmup 25 --name leaf0729_jacanchor_r$r
    uv run python run.py --engines jaseci --url-jaseci http://localhost:8005 \
        --workloads feed_leaf --requests 500 --warmup 25 --name leaf0729_jacleaf_r$r
    echo "=== repeat $r done ==="
done
echo "BENCH ALL DONE"
