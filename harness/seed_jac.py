...
class Seeder:
    ...
    def get_token(self, profile_id):
        ...
        # NOT .local/.test/.example — pydantic's email validator rejects
        # special-use TLDs; any ordinary-looking domain passes (no DNS check).
        email = f"{profile_id}@littlex-bench.com"
        cred = {"type": "password", "password": PASSWORD}
        r = self.client.post("/user/register", json={
            "identities": [{"type": "email", "value": email}],
            "credential": cred,
        })
        # 400 REGISTRATION_FAILED = "... is already registered" on reruns.
        if r.status_code not in (200, 201, 400):
            die(f"register {email}", r)
...
def main():
    ...
    tokens_path = os.path.join(here, "tokens.jaseci.json")
    with open(tokens_path, "w") as f:
        json.dump({username_by_pid[pid]: tok for pid, tok in tokens.items()}, f)
    jids_path = os.path.join(here, "jids.jaseci.json")
